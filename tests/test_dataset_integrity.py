...
class TestDatasetIntegrity(unittest.TestCase):
    ...
    def test_unique_ids_within_files(self):
        for name, data in [
            ...
            self.assertEqual(
                len(ids), len(set(ids)), f"Duplicate IDs found in {name}.json"
            )
    ...
    def test_unique_ids_across_files(self):
        ...
        self.assertEqual(
            len(all_ids),
            len(set(all_ids)),
            "Duplicate IDs found across different dataset files",
        )
            ...
            for item in dataset_map[name]:
                for field in fields:
                    self.assertIn(
                        field,
                        item,
                        f"{name} item {item.get('id')} missing required field: {field}",
                    )
    ...
    def test_supplier_foreign_keys(self):
        for supplier in self.suppliers:
            self.assertIn(
                supplier["port_id"],
                self.port_ids,
                f"Supplier {supplier['business_name']} references invalid port_id",
            )
            for c_id in supplier["commodity_ids"]:
                self.assertIn(
                    c_id,
                    self.commodity_ids,
                    f"Supplier {supplier['business_name']} references invalid commodity_id {c_id}",
                )
    ...
    def test_route_foreign_keys(self):
        for route in self.routes:
            self.assertIn(
                ...
                self.port_ids,
                f"Route {route['route_id']} references invalid origin_port_id",
            )
            self.assertIn(
                route["destination_port_id"],
                self.port_ids,
                f"Route {route['route_id']} references invalid destination_port_id",
            )
    ...
    def test_voyage_foreign_keys_and_capacity(self):
        for voyage in self.voyages:
            self.assertIn(
                voyage["ship_id"],
                self.ship_ids,
                f"Voyage {voyage['id']} references invalid ship_id",
            )
            self.assertIn(
                voyage["route_id"],
                self.route_ids,
                f"Voyage {voyage['id']} references invalid route_id",
            )
            self.assertGreater(
                voyage["total_capacity_ton"],
                0,
                f"Voyage {voyage['id']} total_capacity_ton must be positive",
            )
            self.assertGreaterEqual(
                voyage["used_capacity_ton"],
                0,
                f"Voyage {voyage['id']} used_capacity_ton must be non-negative",
            )
            self.assertGreaterEqual(
                voyage["remaining_capacity_ton"],
                0,
                f"Voyage {voyage['id']} remaining_capacity_ton must be non-negative",
            )
            self.assertLessEqual(
                voyage["used_capacity_ton"],
                voyage["total_capacity_ton"],
                f"Voyage {voyage['id']} used_capacity_ton exceeds total capacity",
            )
            self.assertLessEqual(
                voyage["remaining_capacity_ton"],
                voyage["total_capacity_ton"],
                f"Voyage {voyage['id']} remaining_capacity_ton exceeds total capacity",
            )

    def test_supplier_quality_fields(self):
        for supplier in self.suppliers:
            self.assertGreater(
                supplier["avg_monthly_volume_ton"],
                0,
                f"Supplier {supplier['business_name']} must have positive monthly volume",
            )
            self.assertGreaterEqual(
                supplier["rating"],
                0,
                f"Supplier {supplier['business_name']} rating must be at least 0",
            )
            self.assertLessEqual(
                supplier["rating"],
                5,
                f"Supplier {supplier['business_name']} rating must be at most 5",
            )
            self.assertIsInstance(
                supplier["verified"],
                bool,
                f"Supplier {supplier['business_name']} verified must be boolean",
            )
            self.assertGreater(
                len(supplier["commodity_ids"]),
                0,
                f"Supplier {supplier['business_name']} must supply at least one commodity",
            )

    def test_regulation_documents_ready_for_rag(self):
        filenames = [regulation["filename"] for regulation in self.regulations]
        self.assertEqual(
            len(filenames), len(set(filenames)), "Duplicate regulation filenames found"
        )
        ...
        for regulation in self.regulations:
            ...
            self.assertTrue(
                pdf_path.exists(),
                f"Regulation PDF is missing: {regulation['filename']}",
            )
            self.assertGreater(
                pdf_path.stat().st_size,
                0,
                f"Regulation PDF is empty: {regulation['filename']}",
            )
            self.assertIsInstance(
                regulation["topics"],
                list,
                f"Regulation {regulation['id']} topics must be a list",
            )
            self.assertGreater(
                len(regulation["topics"]),
                0,
                f"Regulation {regulation['id']} must have at least one topic",
            )
    ...
    def test_data_supports_backhaul_candidate_flow(self):
        ...
        for voyage in self.voyages:
            route = next(
                (item for item in self.routes if item["id"] == voyage["route_id"]), None
            )
